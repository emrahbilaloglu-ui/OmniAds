...
export interface AutomationRuleAction {
  ...
  /** Required for the two budget kinds, forbidden everywhere else. */
...
const MAX_BUDGET_CHANGE_PCT = 50;
...
export function validateAutomationRuleDraft(input: {
  ...
  const budgetChangePct = finiteNumber(actionInput.budgetChangePct);
  if (wantsBudgetPct) {
    if (
      budgetChangePct === null ||
      budgetChangePct <= 0 ||
      budgetChangePct > MAX_BUDGET_CHANGE_PCT
    ) {
      throw new AutomationRuleValidationError(
        "action.budgetChangePct",
        "budget_change_pct_must_be_between_0_and_50_exclusive_of_zero",
      );
    }
  } else if (budgetChangePct !== null) {
    throw new AutomationRuleValidationError(
      "action.budgetChangePct",
      "budget_change_pct_is_only_valid_for_budget_actions",
    );
  }
  ...
  return {
    ...
    action: {
      kind: actionKind,
      budgetChangePct: wantsBudgetPct ? budgetChangePct : null,
    },
    mode,
  };
}
...
export function describeAction(action: AutomationRuleAction): string {
  switch (action.kind) {
    ...
    case "propose_budget_decrease":
      return `Propose −${action.budgetChangePct ?? 0}% budget`;
    case "propose_budget_increase":
      return `Propose +${action.budgetChangePct ?? 0}% budget`;
    case "flag_for_review":
      return "Flag for review · no write";
...
/** The map of `AutomationRuleActionKind` onto the proposal contract's action kinds. */
export const AUTOMATION_RULE_PROPOSAL_ACTION_KINDS = {
  propose_pause: "pause",
  propose_budget_decrease: "budget_decrease",
  propose_budget_increase: "budget_increase",
  flag_for_review: "review_flag",
} as const;

export type AutomationProposalActionKind =
  (typeof AUTOMATION_RULE_PROPOSAL_ACTION_KINDS)[keyof typeof AUTOMATION_RULE_PROPOSAL_ACTION_KINDS];
...
export function isWithinQuietHours(
  ...
  const start = trigger.startHour;
  const end = trigger.endHour;
...
export function hourInTimeZone(at: Date, timeZone: string): number | null {
  ...
  try {
    ...
    return Number.isInteger(hour) && hour >= 0 && hour <= 24 ? hour % 24 : null;
  } catch {
    return null;
  }
}
