...
async function main() {
  ...
  try {
    ...
    // The merged global/lane retention kill switch downgrades retention to
    // dry-run, which would make this ownership assertion vacuous. Enable both
    // lanes for this check only and restore them afterwards; the dedicated
    // cleanup-retention-admission tests still cover off/partial-on behaviour.
    const priorGlobalSwitch = process.env.ADSECUTE_SYNC_GLOBAL_ENABLED;
    const priorRetentionSwitch =
      process.env.ADSECUTE_SYNC_LANE_RETENTION_ENABLED;
    process.env.ADSECUTE_SYNC_GLOBAL_ENABLED = "enabled";
    process.env.ADSECUTE_SYNC_LANE_RETENTION_ENABLED = "enabled";
    let pruneSummary;
    try {
      pruneSummary = await pruneMetaCreativeMediaOutsideRetention({
      businessId,
      keepFromDate: AS_OF_DATE,
    });
    } finally {
      if (priorGlobalSwitch === undefined)
        delete process.env.ADSECUTE_SYNC_GLOBAL_ENABLED;
      else process.env.ADSECUTE_SYNC_GLOBAL_ENABLED = priorGlobalSwitch;
      if (priorRetentionSwitch === undefined)
        delete process.env.ADSECUTE_SYNC_LANE_RETENTION_ENABLED;
      else
        process.env.ADSECUTE_SYNC_LANE_RETENTION_ENABLED = priorRetentionSwitch;
    }
    assert(
