...
describe("creative decision engine config-as-data values", () => {
  ...
  it("freezes the PR5-alpha behavior-equivalent values", () => {
    ...
    expect(FATIGUE_FREQUENCY_PRESSURE_QUANTILE).toBe(0.75);
    expect(FATIGUE_STRONG_WINDOW_FALLBACK_ROAS).toBe(1.5);
    expect(LAUNCH_MONITOR_WINDOW_DAYS).toBe(3);
    expect(LIFECYCLE_HELD_REFRESH_CONFIDENCE_CAP).toBe(65);
    expect(TARGET_BAND_MIN_RATIO).toBe(0.85);
    expect(UNCALIBRATED_CUT_RATIO_FALLBACK).toBe(0.7);
    ...
    expect(CALIBRATION_REFIT_INTERVAL_DAYS).toBe(90);
    ...
    expect(BRIEFING_NEAR_MISS_MAX_COUNT).toBe(3);
