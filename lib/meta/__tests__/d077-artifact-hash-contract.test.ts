...
describe("D077 artifact hash contract (fail-closed)", () => {
  ...
  it("the whole-shell stage exists, carries not-applicable counts, and its embedded proof deep-equals a FRESH recomputation from the retained log + CURRENT seam script", () => {
    ...
    /*
      ...
      RELEASE CANDIDATE — repinned to the 2026-09-06 18:50Z capture. These are
      the bytes `bash scripts/verify-database-seams.sh` produced on reviewed
      source freeze 726e9461965f4729b56974e65b717dddf11f005d
      (exit 0, 524.594 s, 40 stages).
    ...
    */
    expect(recomputed.logSha256).toBe(
      "0ea59cc4ef4110e564ebfd6aa86fd95102840d875dd06d2a9f27a61e8c530ab0",
    );
    expect(recomputed.logBytes).toBe(661838);
    /*
