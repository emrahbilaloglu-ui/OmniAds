...
describe("automation rule state machine", () => {
  it("accepts an anchored confirm rule and normalizes its defaults", () => {
    ...
    expect(draft.action).toEqual({ kind: "propose_pause" });
  });
  ...
  it("bounds the anchor multiplier and the day window", () => {
    const base = {
      name: "Bounded",
    ...
    // The action vocabulary is exactly what the confirmation queue can
    // execute. A budget action has no guarded endpoint at any grain, so it is
    // not a kind a rule can name at all.
    expect(() =>
      validateAutomationRuleDraft({
        ...base,
        action: { kind: "propose_budget_increase", budgetChangePct: 20 },
      }),
    ).toThrowError(/unsupported_action_kind/);
    expect(() =>
      validateAutomationRuleDraft({
        ...base,
        action: { kind: "flag_for_review" },
      }),
    ).toThrowError(/unsupported_action_kind/);
  });
});
...
describe("captions", () => {
  ...
  it("describes actions in the design's language and never as an execution", () => {
    expect(describeAction({ kind: "propose_pause" })).toBe(
      "Propose pause into the queue",
    );
    expect(describeAction({ kind: "hard_block_writes" })).toBe(
      "Hard block · logged",
    );
    // The whole vocabulary, so a new kind cannot be added without deciding
    // here what a queue row for it would promise.
    expect([...AUTOMATION_RULE_ACTION_KINDS]).toEqual([
      "propose_pause",
      "hard_block_writes",
    ]);
    // Only the queueable kind maps onto the confirmation queue's own action.
    expect(AUTOMATION_RULE_PROPOSAL_ACTION_KINDS).toEqual({
      propose_pause: "pause",
    });
  });
});
