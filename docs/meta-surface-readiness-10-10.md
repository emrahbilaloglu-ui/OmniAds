...
| Landing Pages | 4 | NO — rule engine runs in the browser | store-sourced, not payload | cached, no as-of; unavailableMetrics swallowed |
...
1. **Landing Pages decision engine is client-side.** `lib/landing-pages/rule-engine.ts`
   computes scale/keep/investigate with hardcoded thresholds in the browser
   (`rule-engine.ts:260`, bands `:63-74`) and its output feeds the server AI
   commentary endpoint. Move the rule report server-side (or make its output
   a server contract); add route tests (currently none).
2. **Creative Inbox / BriefingCreativeCard has no currency field.** The inbox
...
3. **Copies API omits funnel/video metrics for copy rows**, forcing the page
...
4. **Anomaly rows carry no per-entity briefing status**, so the anomalies
...
5. **Meta v1 recommendation engine lacks the CDC disciplines** (no
...
6. **Launchpad payload contract has no currency code** (minor-units only)
   and `lib/launchpad/meta.ts` (637-line payload normalizer) plus the
   templates/drafts routes are untested.
7. **Two-source freshness model on the Decision Center** (live pulse
...
8. **Landing Pages / Copies payloads lack generatedAt/as-of fields**;
   route-report caches serve data with no age indication.
9. **Present-config-over-history classes in lib/meta/serving.ts** (historical
   windows classified by current status/bid config) — anachronism debt
   shared by pulse and lanes.
10. **Stale e2e specs assert dead testids** (`reviewer-smoke.spec.ts:42`,
...
- Decision Center: drill-drawer KPI header aggregates display strings
  (display-only); bid overlay display value regex fallback; remaining
  legacy formatCurrency in drill drawer + bid notices; "Let cook" label is
  client-conditional (honest, but client-decided); compare drawer visuals.
...
- Landing Pages: unrendered `unavailableMetrics` (rendering is UI; the
  missing field semantics are NON-UI #8); ternary i18n; typo "adjüsting";
  gradient hero.
...
- Every decision/action label traces to a server-computed field guarded by
  an invariant test (Decision Center: done; Landing Pages: open).
