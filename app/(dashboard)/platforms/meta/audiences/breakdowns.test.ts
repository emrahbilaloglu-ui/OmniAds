...
describe("the audience breakdown panels bind the served rows", () => {
  ...
  it("regroups platform from the placement key the server produced", () => {
    const platform = buildAudienceBreakdowns(payload).find((p) => p.id === "platform");
  ...
  it("scopes the gender and frequency gap to the range rather than calling it permanent", () => {
    ...
    expect(gender).toMatch(/for this range/i);
    expect(frequency).toMatch(/reach/i);
    expect(frequency).toMatch(/for this range/i);
  ...
  it("draws the gender panel once the range carries the split", () => {
    const built = buildAudienceBreakdowns({
      ...payload,
      gender: [
        { key: "female", label: "Female", spend: 600, purchases: 12, revenue: 1800, clicks: 900, impressions: 40000 },
        { key: "male", label: "Male", spend: 400, purchases: 7, revenue: 900, clicks: 610, impressions: 26000 },
      ],
    });
   ...
   * measurement. The rows stay — they are real — and the note carries the
   * served reason onto the panels that drew them.
   */
  it("carries the served reason onto the panels while a range is still partial", () => {
    ...
    expect(buildAudienceBreakdowns(payload).find((p) => p.id === "age")?.note).toBeNull();
  });
   ...
   * reason the route (or the fetch) gave is carried onto every panel.
   */
  it("states the reason on every panel when the read failed or was refused", () => {
    ...
    expect(refused.every((p) => p.note === "The requested Meta ad account is not assigned to this workspace.")).toBe(true);

    const failed = buildAudienceBreakdowns(null, "Meta breakdowns could not be read (500).");
    expect(failed.every((p) => p.note === "Meta breakdowns could not be read (500).")).toBe(true);
  });
