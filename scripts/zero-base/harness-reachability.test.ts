...
const PERMITTED_REFERENCE_BODIES: Record<string, string> = {
  "components/zero-base/_reference/home-view.tsx":
    "Repointing needs four Overview FEATURES that do not exist, not four fakes. H03/H04/B01 require data-el=source-readiness with data-collection=sources, data-ctl=live:INTEGRATION-03 connect, live:chart-table-toggle, live:ECON-04 divergence-link and live:MOBILE-01; app/(dashboard)/overview/legacy-page.tsx has no source-health panel, no chart/table toggle, no break-even divergence link and no mobile triage entry anywhere in it. Attaching the markers to nothing would be stamping the contract; building the four is a product decision about what Overview IS, which the master plan does not make. The presenter extraction on top of that is real but secondary.",
  "components/zero-base/_reference/meta-decisions-view.tsx":
    "The mounted Decision Center is a different product from the one the design draws, and the repoint would have to build the difference. MetaDecisionCenterExact.tsx carries ZERO data-el/data-ctl/data-collection today and, more importantly, has no element to attach nine of them to: its lanes are action|watching|healthy|nonsales|archive with no Needs Resolution lane (so no blocker-chip and no needsres collection), it filters by scope (structure|creatives) and not by LEVEL (so no live:META-DEC-02 level), its inspector is an inline grid column with no close control (so no live:close), and it has no share-view, no brief, no Ads Manager open, no mutation-ceremony trigger, no evidence-window statement and no stale-demoted marker. It also has no conflict UI at all: use-decision-workflow.ts:182-193 handles a 409 and sets lastMessage, and MetaPlatformPage never reads it — so H12's conflict-dialog with keep/reapply is a feature to build, not a marker to move. And every string in the file is a hardcoded English literal with no useCopy, so the Turkish artboards P06/P07 cannot render from it without routing the whole surface through the copy module. Each of those is a decision about what the Decision Center should BE.",
};
