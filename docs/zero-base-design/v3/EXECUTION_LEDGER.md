...
| 02 | **complete** | `792c8a213` | `scripts/zero-base/{generate,verify}-contracts.ts`, `lib/zero-base/{generated-contracts,route-registry,control-registry,report-source-registry,contract}.ts` | `test:zero-base:contract` 17/17 · `zero-base:contracts:check` current · typecheck 0 · lint 0 | — | `git revert 792c8a213` | gates proven fail-closed by tamper test: leaf-id edit → check stale + 3 test failures; `audit.json` `NOT READY`→`READY` → hash-drift + verdict failure |
