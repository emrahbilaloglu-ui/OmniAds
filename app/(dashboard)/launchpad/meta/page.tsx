...
interface LaunchpadRecentAdAction {
...
function resolveLaunchpadAdActionId(row: MetaCreativeRow) {
  return row.realAdId?.trim() || row.id;
}

function applyRecentAdActionsToRows(
  ...
  const syntheticRows = actions.flatMap((action): MetaCreativeRow[] => {
    ...
    const source = action.creativeId ? sourceByCreativeId.get(action.creativeId) : null;
    if (!source && !action.creativeId) return [];
    ...
    return [
      {
        ...(source ?? {
          ...
          creativeId,
          name: action.adName ?? `Meta ad ${action.resultingAdId}`,
        ...
        creativeId,
        name: action.adName ?? source?.name ?? `Meta ad ${action.resultingAdId}`,
