...
export function MiniTrendAreaChart({
  ...
  data: Array<{ date: string; value: number }>;
  /**
   * Both call sites survive here: the v2 cards pass `tone`, the legacy overview
   * cards pass `label`. Neither is drawn — the sparkline colours itself from the
   * series and the surrounding card already carries the name — so both stay
   * optional rather than forcing one family of callers to change.
   */
