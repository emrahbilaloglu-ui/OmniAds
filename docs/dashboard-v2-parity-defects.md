...
| KLAVIYO-INTEGRATIONS-28 | BLOCKED | Klaviyo has no authorization route: `app/api/oauth/klaviyo/start/route.ts` answers 501 by design. The card therefore renders no button (the design's own `showBtn` conditional) and its meta line reads the design's own non-connected string, "no data pulled yet" (see F2). **Contract to unblock:** a real Klaviyo OAuth start/callback pair persisting an integration row, so the card can show "Connect" and the first-sync block can run. |
| KLAVIYO-INTEGRATIONS-29 | BLOCKED | Nothing serves Klaviyo lifecycle flows: there is no `app/api/klaviyo/**` route and no Klaviyo warehouse table. Per H3 the table keeps its five headers and one row of the design's row geometry with "—" in every cell. **Contract to unblock:** a route returning `{ id, name, status, revenue28d, openRate, recipients }` per flow for a business; `KlaviyoFlowSource` in `klaviyo-exact-adapter.ts` is that shape already, and `klaviyo-client.tsx` has one line to change. |
