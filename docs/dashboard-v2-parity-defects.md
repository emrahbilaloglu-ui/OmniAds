...
| INSIGHTS-A-37 | BLOCKED | Revenue still prints a hardcoded `$`. GA4 returns `purchaseRevenue` in the property's own currency and none of the **six** endpoints that serve it expose that currency, so a EUR/TRY property is labelled in dollars. **Contract required:** `/api/analytics/overview`, `/products`, `/audience`, `/demographics`, `/cohorts` **and `/api/geo/traffic-sources`** must each return the GA4 property's ISO-4217 currency, after which the adapter can call `formatMoneyIso` and render `—` when the code is unknown. **Widened by batch 11:** `/api/geo/traffic-sources` is the sixth endpoint with this gap. It requests `purchaseRevenue` (`app/api/geo/traffic-sources/route.ts:63`) and `buildGeoSources` renders it with `formatCurrencySmart(revenue, "$")` (`components/geo/insights-geo-exact-adapter.ts:480-483`) in the AI Sources tab's Revenue column, which this batch introduced. Same defect, same fix, now listed. **Corrected by batch 11:** the currency is *not* already resolved. `getGA4TokenAndProperty` (`lib/google-analytics-reporting.ts:374-391`) calls no Admin API at all — it reads `resolveGa4AnalyticsContext`, which returns only what is stored in the integration metadata (`ga4PropertyId`, `ga4PropertyName`, `ga4PropertyTimeZone`). A `properties/{id}` GET does exist (`lib/google-analytics-accounts.ts:185-218`, `fetchGA4PropertyMetadata`) but reads `timeZone` only and runs once, at property selection (`app/api/google-analytics/select-property/route.ts:249`). Closing this therefore needs three steps, not one: read `currencyCode` from that Admin call, persist it beside `ga4PropertyTimeZone`, and return it from the five endpoints. |
...
| INSIGHTS-B-46 | BLOCKED | **Raised by verification.** The AI Sources Revenue column this batch introduced renders GA4 `purchaseRevenue` with a hardcoded `$` — the identical defect INSIGHTS-A-37 documents, on a sixth endpoint A-37's contract did not list. `/api/geo/traffic-sources` is now named in that contract (both the status row and the A-37 block), so a EUR or TRY property being labelled in dollars here is a disclosed instance of one open contract rather than an unlisted new one. Rendering `—` was rejected: it would blank a real served number on this surface while the five analytics surfaces keep printing theirs, which is a worse asymmetry than the disclosed substitution. Closes with A-37. |
