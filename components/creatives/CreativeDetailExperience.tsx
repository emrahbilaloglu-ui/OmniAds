...
export function CreativeDetailExperience({
  ...
  const shouldFetchHtmlPreview =
    open && Boolean(businessId) && Boolean(row?.creativeId) && canRequestHtml;
  ...
  const detailPreviewQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return typeof detail?.html === "string" && detail.html.trim().length > 0
        ? detail.html
        : null;
    },
  });

  const currency = resolveCreativeCurrency(
    row?.currency ?? null,
    defaultCurrency,
  );
  ...
  return (
    <div className="fixed inset-0 z-[90]">
      ...
      <div className="absolute inset-2 overflow-hidden rounded-xl border border-neutral-200 bg-neutral-50 shadow-[0_8px_24px_-12px_rgba(16,21,28,0.18)] md:inset-4">
        <header className="flex h-16 items-center justify-between border-b border-neutral-200 bg-white/95 px-4 backdrop-blur md:px-6">
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-neutral-500">
              {formatCreativeDateLabel(dateRange)}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <DateRangePicker
              ...
              onChange={(next) =>
                onDateRangeChange(standardDateRangeToCreative(next))
              }
        ...
        */}
        <div className="grid h-[calc(100%-64px)] grid-cols-1 lg:grid-cols-[minmax(0,1.7fr)_minmax(340px,460px)]">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-100 px-4 py-3">
                  <div className="flex flex-wrap gap-1.5">
                    {taxonomyPills.primaryLabel ? (
                      <Pill value={taxonomyPills.primaryLabel} />
                    ) : null}
                    {taxonomyPills.secondaryLabel ? (
                      <Pill value={taxonomyPills.secondaryLabel} />
                    ) : null}
                    {row.launchDate ? (
                      <Pill value={`Launched ${row.launchDate}`} />
                    ) : null}
                  </div>
                </div>
                        ...
                        <div className="space-y-3">
                          <div className="relative flex max-h-[78vh] w-full max-w-[860px] items-center justify-center overflow-hidden p-2">
                            <img
                              src={imageUrl}
                              alt={row.name}
                              className="relative z-[1] block max-h-[74vh] w-auto max-w-full object-contain"
                            />
                          </div>
                        </div>
                      ) : canRequestHtml ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                >
                  <PrimaryMetricTile
                    label="Spend"
                    value={formatMoney(row.spend, currency, defaultCurrency)}
                  />
                  <PrimaryMetricTile
                    label="ROAS"
                  ...
                  <PrimaryMetricTile
                    label="Purchases"
                    value={formatInteger(row.purchases)}
                  />
                  <PrimaryMetricTile
                    label="CTR"
                ...
                <div className="grid grid-cols-2 gap-x-3.5 gap-y-1.5 text-[12px] tabular-nums">
                  ...
                  <SecondaryMetricRow
                    label="Impressions"
                    value={formatInteger(row.impressions)}
                  />
                  <SecondaryMetricRow
                    label="Link clicks"
                    value={formatInteger(row.linkClicks)}
                  />
                </div>
              </div>
...
function Pill({ value }: { value: string }) {
  return (
    <span className="rounded-full border border-neutral-200 bg-white px-2.5 py-1 text-[10px] font-medium text-neutral-600">
      {value}
    </span>
  );
}
...
function PrimaryMetricTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-1 bg-white px-3 py-2.5">
      ...
      <span className="text-[18px] font-semibold leading-none tracking-tight text-neutral-900">
        {value}
      </span>
    </div>
  );
