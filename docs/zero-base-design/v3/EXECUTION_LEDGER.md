...
| 10 | **complete** | `15de2901c` (projection) → `45a9ff1ab` (routes + Flow A) | `lib/zero-base/{agency-projection,agency-directory-server}.ts`, `components/zero-base/agency/**`, `app/a/desk/{page,clients,withheld}`, `scripts/zero-base/build-shell-harness.tsx` | 49 tests (18 projection · 17 view · 7 server · 7 rollout) · 4 Playwright 50-client checks · full suite 7971/0 | 50-client scan at 1440 and 390 in both themes: all 50 rows, no page overflow, no money/ranking text | `git revert 45a9ff1ab 15de2901c`; rollout OFF makes `/a` not found before any read | Two queries regardless of client count; legacy projection retained until WP-27. Load-more now disables with its reason instead of disappearing |
...
- Phase B (WP-04 – WP-10) ran from `33df99879` and is **complete**. WP-09 and WP-10 were partial at `66dbfc3fb` and were finished by the follow-up commits `45276d383` and `45a9ff1ab`; the partial commits are retained and are not relabelled. Full evidence: `PHASE_B_IMPLEMENTATION_REPORT.md`.
