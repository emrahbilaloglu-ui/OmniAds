...
describe("MetaDecisionCenterExact branches and callbacks", () => {
  ...
  it("shows measured CTR with or without a daily trail and leaves an unserved CTR slot absent", () => {
    ...
    const measuredWithTrail = document.querySelector('[data-meta-exact-creative-row="measured-with-trail"]');
    ...
    expect(metrics?.textContent).toContain("Decision");
    expect(metrics?.textContent).not.toContain("Decision · 28d");
    expect(measured?.querySelector('[data-meta-exact-creative-ctr-value]')?.textContent).toBe("0.00%");
    expect(measured?.textContent).toContain("CTR");
    expect(measured?.textContent).not.toContain("CTR · 28d");
