...
| KLAVIYO-INTEGRATIONS-31 | BLOCKED | GA4 and Search Console serve no sync-status endpoint, so no first-import progress is observable for them and the block is never shown on their cards (rather than faked). **Contract to unblock:** `/api/ga4/status` and `/api/google-search-console/status` reporting the same facts the adapter's `FirstSyncSignals` needs (connected, in flight, entities discovered, backfill fraction, snapshot ready, connect date). |
