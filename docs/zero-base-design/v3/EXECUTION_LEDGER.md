...
| 10 | **complete** | `15de2901c` (projection) → `45a9ff1ab` (routes + Flow A) → `601094543` · `0a50fa6a8` (**server pagination correction**) | `lib/zero-base/{agency-projection,agency-directory-store}.ts`, `app/api/agency-today/route.ts` (`contract=zero-base.v1` branch), `components/zero-base/agency/**`, `app/a/desk/{page,clients,withheld}`, `scripts/ephemeral-postgres-agency-directory-seam-child.ts` | 18 projection · 25 view · 12 boundary · **real-PostgreSQL seam over 121 clients** · full suite 7984/0 | 50-client visible scan at 1440/390 both themes; seam proves 121-client keyset paging at 3 page sizes | `git revert 0a50fa6a8 601094543 45a9ff1ab 15de2901c`; rollout OFF 404s the boundary before any read | **`45a9ff1ab` shipped client pagination behind an unbounded server read and the ledger wrongly called it server-paginated.** `601094543` replaces it with SQL keyset pagination (C collation, name+id total order); the superseded reader is deleted. Legacy projection retained until WP-27 |
...
- **Correction after acceptance review:** `45a9ff1ab` implemented the Agency directory as an unbounded server read plus client-side slicing, while this ledger claimed server pagination. That claim was wrong. `601094543` implements genuine SQL keyset pagination and adds a real-PostgreSQL seam over 121 clients, and `0a50fa6a8` replaces the layout's materialise-to-count gate with a bounded count query. The earlier commits are retained in history rather than rewritten. Full evidence: `PHASE_B_IMPLEMENTATION_REPORT.md`.
