 ...
 * same warehouse day a no-op rather than a second queue entry.
 */
  ...
  for (const verdict of input.verdicts) {
    ...
    if (verdict.outcome === "proposal") {
      ...
      if (actionKind) {
        ...
        const receipt = await sink(draft);
