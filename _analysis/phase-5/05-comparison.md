# Phase 5 — 5-way comparison: v3 / me / Marcus / Dr. Lin / Aria

Universe: **154 creatives** (TheSwaf 47 + IwaStore 107; active + closed_30d).

## Distribution per labeler

| Label | v3 | me | Marcus | Dr. Lin | Aria |
|---|---:|---:|---:|---:|---:|
| scale | 1 | 1 | 13 | 1 | 13 |
| keep | 18 | 7 | 28 | 23 | 34 |
| refresh | 0 | 4 | 18 | 0 | 17 |
| cut | 5 | 24 | 22 | 2 | 57 |
| test_more | 99 | 96 | 73 | 128 | 0 |
| diagnose | 17 | 22 | 0 | 0 | 33 |
| out_of_scope | 5 | 0 | 0 | 0 | 0 |
| missing | 9 | 0 | 0 | 0 | 0 |

## Pairwise agreement (excluding missing/out_of_scope)

| | v3 | me | marcus | lin | aria |
|---|---:|---:|---:|---:|---:|
| v3 | 100.0% | 73.6% | 41.4% | 74.3% | 12.1% |
| me | 73.6% | 100.0% | 55.8% | 68.2% | 24.0% |
| marcus | 41.4% | 55.8% | 100.0% | 50.0% | 24.7% |
| lin | 74.3% | 68.2% | 50.0% | 100.0% | 3.2% |
| aria | 12.1% | 24.0% | 24.7% | 3.2% | 100.0% |

## Strong consensus (5/5 same label) — 0 creatives


## 4/5 consensus (1 outlier) — 58 creatives

| business | name | bucket | spend_28d | roas_28d | v3 | me | Marcus | Lin | Aria | outlier |
|---|---|---|---:|---:|---|---|---|---|---|---|
| TheSwaf | EMB - CatalogAd | active | $9963 | 1.3732 | keep | cut | cut | cut | cut | **v3** |
| TheSwaf | EmB - Catalog Ad | active | $8824 | 1.3108 | keep | cut | cut | cut | cut | **v3** |
| TheSwaf | wearthefearrevise | active | $2241 | 0.8885 | cut | cut | cut | test_more | cut | **lin** |
| TheSwaf | depth | closed_30d | $587 | 0.6284 | cut | cut | cut | test_more | cut | **lin** |
| TheSwaf | restraintrevise | active | $504 | 4.1756 | scale | scale | scale | keep | scale | **lin** |
| TheSwaf | watchthat | closed_30d | $226 | 0.0000 | cut | cut | cut | test_more | cut | **lin** |
| IwaStore | PrayerEssentialsCatalog | active | $72 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | The heart finds | active | $49 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| TheSwaf | Tastemythrevise | active | $48 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | WallArtCatalog | active | $40 | 0.0000 | test_more | test_more | test_more | test_more | diagnose | **aria** |
| TheSwaf | EMB - ReligiousNecklace | active | $33 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| TheSwaf | bestsellersnecklace | active | $32 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| TheSwaf | softness | active | $30 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | Iconic beauty | closed_30d | $4 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | Timeless Crescent | closed_30d | $2 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| TheSwaf | EMB - AllRings | active | $13 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | Transforming a house | active | $16 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | Gift a sanctuary | closed_30d | $1 | 0.0000 | test_more | test_more | test_more | test_more | keep | **aria** |
| TheSwaf | whispersfromthedevil | active | $11 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | A soulful gift | closed_30d | $1 | 0.0000 | test_more | test_more | test_more | test_more | keep | **aria** |
| TheSwaf | EMB - Mix | active | $5 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | Decorating the | closed_30d | $1 | 0.0000 | test_more | test_more | test_more | test_more | keep | **aria** |
| IwaStore | Glowing lanterns | closed_30d | $7 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | decorista_93 | active | $7 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |
| IwaStore | It became the favorite | active | $6 | 0.0000 | test_more | test_more | test_more | test_more | cut | **aria** |

_... and 33 more_

## High disagreement (≥4 unique labels) — 7 creatives

| business | name | bucket | spend_28d | roas_28d | v3 | me | Marcus | Lin | Aria |
|---|---|---|---:|---:|---|---|---|---|---|
| IwaStore | WallArtCatalog | closed_30d | $865 | 3.9251 | keep | diagnose | refresh | scale | diagnose |
| IwaStore | WoodenWallArtCatalog | active | $5 | 31.2818 | diagnose | test_more | keep | test_more | scale |
| TheSwaf | EMB - AllRings | active | $498 | 1.4537 | keep | cut | refresh | test_more | refresh |
| TheSwaf | TargeryenFlex | closed_30d | $0 | 1085.5263 | test_more | cut | scale | keep | diagnose |
| IwaStore | A prayer niche | closed_30d | $116 | 3.2574 | test_more | diagnose | refresh | keep | diagnose |
| IwaStore | Since I hung | closed_30d | $128 | 2.2998 | test_more | diagnose | refresh | keep | refresh |
| TheSwaf | Mix | closed_30d | $3 | 0.0000 | test_more | diagnose | scale | keep | diagnose |

## v3 vs human consensus

- **v3 matches majority consensus**: 101/154 (65.6%)
- **v3 differs from majority**: 33/154 (21.4%)
- **No clear consensus among humans+engine**: 20/154 (13.0%)

