...
counts AS (
  SELECT
    ...
    (SELECT COUNT(*) FROM mature) AS mature_creative_count,
...
percentiles AS (
  SELECT
    (SELECT percentile_cont(0.75) WITHIN GROUP (ORDER BY aggregate_roas) FROM mature WHERE aggregate_roas IS NOT NULL) AS raw_roas_p75,
    (SELECT percentile_cont(0.60) WITHIN GROUP (ORDER BY aggregate_roas) FROM mature WHERE aggregate_roas IS NOT NULL) AS raw_roas_p60,
    (SELECT percentile_cont(0.10) WITHIN GROUP (ORDER BY recent_total_ratio) FROM recent_ratios WHERE recent_total_ratio IS NOT NULL) AS raw_refresh_ratio_p10,
    (SELECT percentile_cont(0.10) WITHIN GROUP (ORDER BY cumulative_28d_ctr) FROM per_creative WHERE cumulative_28d_ctr IS NOT NULL) AS raw_low_ctr_p10
),
source_bounds AS (
  SELECT
...
SELECT
  ...
  counts.mature_creative_count,
  counts.zero_conversion_count,
  CASE WHEN counts.mature_creative_count >= 30 THEN percentiles.raw_roas_p75 END AS roas_p75,
  CASE WHEN counts.mature_creative_count >= 10 THEN percentiles.raw_roas_p60 END AS roas_p60,
  CASE WHEN counts.refresh_ratio_count >= 20 THEN percentiles.raw_refresh_ratio_p10 END AS refresh_ratio_p10,
  CASE WHEN counts.ctr_count >= 20 THEN percentiles.raw_low_ctr_p10 END AS low_ctr_p10,
...
INSERT INTO engine_v3_account_calibration_daily (
  ...
  $17::date, $18::date, $19::timestamptz,
  $20, $21::uuid, $22::timestamptz
)
