...
| LAUNCHPAD-AUTOMATION-30 | CLOSED | `meta_automation_proposals` is the single confirmation queue. It projects each snapshot's `cut` decisions AND receives rule firings through the intake sink, discriminated by `origin`; `/api/meta/automation/proposals` approves through the existing `handleMetaEntityPauseAction` guarded path and modifies/dismisses without one. |
| LAUNCHPAD-AUTOMATION-31 | CLOSED | `meta_automation_rules` / `_rule_firings` land the typed definition, the real 28-day count and the guarded toggle + `+ New rule` (`create_rule` / `set_rule_active` / `evaluate_rules` on `/api/meta/automation`, `collaborator` floor, reviewer read-only). Triggers anchor to the target pack — never an absolute number — and the only outcomes are a `pending` row in `meta_automation_proposals` or a hard block; proof in `lib/meta/automation-rules.test.ts`, `automation-rules-store.test.ts`, `automation-rules-evaluation.test.ts`, `app/api/meta/automation/rules-route.test.ts`, `lib/meta/automation-single-write-path.test.ts` and the Automation surface tests. |
...
| ~~B1~~ DELIVERED | Automation rules | Typed definitions, trigger/then/mode, 28-day fired count, locked state and guarded toggle/new-rule mutations. | Served by `MetaAutomationControlPlane.rules` + `readCompleteness.rules`. Rows render only on a proven-complete read; an unproven read keeps the em-dashed five-column shell, and a proven-empty table is an honest empty state — no seeded example rules. A firing raises a `pending` row into the same `meta_automation_proposals` queue and never reaches a provider. |
...
`lib/meta/automation-proposals.test.ts` (state machine, expiry, the single-queue
projection guard and the rule intake), `lib/meta/automation-proposal-execution.test.ts`
(the reuse of the guarded handler, and a source assertion that no second write
path is imported), `app/api/meta/automation/proposals/route.test.ts`
(authorization boundary, kill switch, Tier‑1 proof, confirmation token, dry-run
guardrail), `automation-proposals-exact-adapter.test.ts`, and three
`lib/meta/snapshot.test.ts` cases pinning the per-snapshot re-projection. The
rules engine adds `lib/meta/automation-rules.test.ts` (state machine, purity and
the action vocabulary), `automation-rules-store.test.ts`,
`automation-rules-evaluation.test.ts`, `automation-rules-exact-adapter.test.ts`
and `app/api/meta/automation/rules-route.test.ts`. Both halves are pinned
together by `lib/meta/automation-write-path.test.ts`, which asserts that exactly
one module in the subsystem reaches the guarded handler and that a rule firing
...
**Design lines read:** Klaviyo markup 1694-1733; Integrations markup 2815-2879; model 3178 (`sync`, `syncDone`, `klaviyoOn`), 3214-3226 (`startSync`), 3284 (sidebar gate + BETA badge), 3895-3903 (`klaviyoTabs`, `flows`), 4315-4321 (`integBase`), 4323-4330 (`syncSteps`), 4332-4348 (`integrations`), 4349-4353 (`soonIntegrations`).

**Shape:** `components/integrations/IntegrationsExact.tsx` + `.module.css` + `integrations-exact-adapter.ts` + `integrations-exact-model.ts`; `components/klaviyo/KlaviyoExact.tsx` + `.module.css` + `klaviyo-exact-adapter.ts` + `klaviyo-client.tsx`. Routes: `app/(dashboard)/integrations/legacy-page.tsx` (also mounted by `/c/{businessId}/manage/integrations` and `/app/manage/integrations`), `app/(dashboard)/platforms/klaviyo/page.tsx`, new `app/c/[businessId]/klaviyo/page.tsx` and the `/app/klaviyo` dispatch entry.

**Backend wiring.** The first-sync block is driven entirely by served provider status, never a timer: `/api/meta/status` (`connected`, `assignedAccountIds`, `priorityWindow.completedDays/totalDays`, `currentCoreProgressPercent`, `coreReadiness.complete`, `state`), `/api/google-ads/status` (`assignedAccountIds`, `historicalProgress.percent`, `backgroundBackfill.percent`, `state`), `/api/shopify/status` (`shopId`, `sync.ordersHistorical.readyThroughDate` vs `historicalTargetEnd`, `state`). The percent never runs ahead of evidence — an unknown backfill parks the bar at the start of its own stage — and the block disappears the moment the provider reports a landed snapshot, which is the design's "appears once" rule. Its *existence* is bound to the same standard: see F1 for the five facts that must all hold before a card may claim a first import at all.

| id | state | proof |
| --- | --- | --- |
| KLAVIYO-INTEGRATIONS-01 | CLOSED | `IntegrationsExact.tsx` renders a card with three children (head, description, footer) plus the single first-sync block. `IntegrationsExact.test.tsx` asserts `card.children` is 3 without a first sync, and the source test asserts `StateBanner`/`MetaIntegrationProgress`/`GoogleIntegrationProgress`/`ShopifyIntegrationStatus`/`SyncStatusPill`/`syncNotice` appear nowhere. The two progress components were deleted. |
| KLAVIYO-INTEGRATIONS-02 | CLOSED | The model carries one nullable `button`. `IntegrationsExact.test.tsx` asserts one `<button>` per card; `page.test.tsx` asserts a connected Meta card has exactly one, captioned "Manage". Disconnect moved into the Manage destinations (assignment drawer, GA4 picker, Search Console dialog); Reconnect/Retry are subsumed by the design's own Connect caption on an unhealthy connection; Cancel is subsumed by the design showing no button while the import runs. |
| KLAVIYO-INTEGRATIONS-03 | CLOSED | `FirstSyncBlock` in `IntegrationsExact.tsx` (radius 11px, `#f7f9fc`, 1px `#e4e8f0`, 11px/13px pad, mono "First sync", mono percent `#2f6bff`, 6px bar `#e4e8f0`/`#2f6bff`/`#0e9f6e`, four 15px check circles). `resolveFirstSyncPercent` + the per-provider signal readers are unit-tested; `page.test.tsx` renders 59% from a served `priorityWindow` of 14/28 days and asserts the block disappears at `state: "ready"`. |
| KLAVIYO-INTEGRATIONS-04 | CLOSED | `components/integrations/connect-modal.tsx` deleted. `handleCardAction` sets `window.location.href` to `getOAuthStartUrl(...)` directly; `page.test.tsx` asserts the click reaches `/api/oauth/google/start?businessId=biz_1` with no "Requested permissions" step. |
| KLAVIYO-INTEGRATIONS-05 | CLOSED | The header renders eyebrow, `h1` and one paragraph and nothing else. `page.test.tsx` asserts the business name and the "demo fixtures" chip are absent. |
| KLAVIYO-INTEGRATIONS-06 | CLOSED | `StateBanner` and the whole `toast`/`setToast`/`clearToast` path are gone from the route body. |
| KLAVIYO-INTEGRATIONS-07 | CLOSED | `.card` is `#e4e8f0`; `.cardSyncing` is the only override, `#cbd9ff`. No state-coloured border remains. |
| KLAVIYO-INTEGRATIONS-08 | CLOSED | `app/(dashboard)/platforms/klaviyo/page.tsx` renders `<KlaviyoClient />` with no wrapper. `KlaviyoExact.test.tsx` asserts `ad-workspace-page` and `max-width: 1060px` appear in neither the component nor its stylesheet. |
| KLAVIYO-INTEGRATIONS-09 | CLOSED | `.cardButton` is `height: 30px; padding: 0 12px; border-radius: 8px; font-size: 12px; font-weight: 600` with no min-width; the source test asserts `min-width: 104px` is absent. |
| KLAVIYO-INTEGRATIONS-10 | CLOSED | `.statusPill` is `inline-flex; gap: 6px` with a `.statusDot` 5×5 `currentColor` span as its first child, in every tone. |
| KLAVIYO-INTEGRATIONS-11 | CLOSED | The adapter captions a connected card "Manage" for every provider; unit test walks shopify/meta/google/ga4/search_console. `providerActionLabel` in `store/integrations-support.ts` still returns the old provider-specific strings but no UI reads `primaryActionLabel` any more. |
| KLAVIYO-INTEGRATIONS-12 | CLOSED | `INTEGRATIONS_LIVE_ORDER` is shopify, meta, google, ga4, search_console, klaviyo; asserted by both the adapter test and `page.test.tsx` against rendered `data-provider` order. |
| KLAVIYO-INTEGRATIONS-13 | CLOSED | `INTEGRATIONS_PROVIDER_NAMES` and `PROVIDER_LABELS` in `components/integrations/oauth.ts` both read Meta Ads / Google Ads / Search Console / TikTok Ads. |
| KLAVIYO-INTEGRATIONS-14 | CLOSED | `INTEGRATIONS_PROVIDER_DESCRIPTIONS` carries the design's six sentences verbatim; two are asserted character-for-character in the adapter test. |
| KLAVIYO-INTEGRATIONS-15 | CLOSED | `.cardFoot` is one `margin-top: auto` row of meta span, `flex: 1` spacer, button. `IntegrationsExact.test.tsx` asserts the meta text and the button share the card's third child. |
| KLAVIYO-INTEGRATIONS-16 | CLOSED | The four sub-route files are deleted, their `screen-registry.ts` entries and `/app/**` aliases removed, and `getPlatformLayer2Items("klaviyo")` returns `[]` so the rail links to `/platforms/klaviyo`. `app/app/klaviyo-route-dispatch.test.tsx` asserts the four leaf spellings 404. |
| KLAVIYO-INTEGRATIONS-17 | CLOSED | `IntegrationsExactSkeleton` renders the real header plus one `repeat(auto-fit,minmax(300px,1fr))` grid of six card outlines; the three summary tiles and three titled sections are gone. |
| KLAVIYO-INTEGRATIONS-18 | CLOSED | `.card { gap: 10px }`. |
| KLAVIYO-INTEGRATIONS-19 | CLOSED | `.table { font-variant-numeric: tabular-nums }`, asserted in `KlaviyoExact.test.tsx`. |
| KLAVIYO-INTEGRATIONS-20 | CLOSED | `.header { flex-wrap: wrap }`, asserted in `KlaviyoExact.test.tsx`. |
| KLAVIYO-INTEGRATIONS-21 | CLOSED | `resolveStatus` returns "Connecting" both while the first import runs and for `loading_data`; "Loading" no longer exists on the surface. |
| KLAVIYO-INTEGRATIONS-22 | CLOSED | `.footNote` sets `--font-ibm-plex-mono`, asserted by regex in `KlaviyoExact.test.tsx`. |
| KLAVIYO-INTEGRATIONS-23 | CLOSED | The provider name is a `<span>`; both the component test and the route test assert exactly one `h2` on the screen and that it reads "Coming soon". |
| KLAVIYO-INTEGRATIONS-24 | CLOSED (geometry) / BLOCKED (action) | New finding. The roadmap card had no "Notify me" button at all (design 2872: h28, 0 11px pad, 8px radius, 1px `#e4e8f0`, 11.5px/600 `#45526b`). It is now rendered with that geometry and `disabled`. **Contract to unblock:** a notify-me subscription endpoint (e.g. `POST /api/integrations/notify` taking `{businessId, provider}`) plus storage for the subscription. |
| KLAVIYO-INTEGRATIONS-25 | CLOSED | New finding. `screen-registry.ts` has always claimed `/app/klaviyo` and `/c/{businessId}/klaviyo` as the canonical Klaviyo family, but no page existed at either address, so the rail's Klaviyo entry 404'd on the canonical shell. `app/c/[businessId]/klaviyo/page.tsx` now exists behind `getSessionFromCookies` + `requireBusinessPageContext`, and `/app/klaviyo` dispatches to it. |
| KLAVIYO-INTEGRATIONS-26 | CLOSED | New finding, H1 violation. `lib/klaviyo/service.ts` fabricated flow revenue ($28,400 Welcome Series, $41,850 Abandoned Cart), open rates, benchmarks and "AI" recommendations, feeding `components/klaviyo/KlaviyoDashboard.tsx` — a component no route mounted. All four files deleted (`service.ts`, `types.ts`, `KlaviyoDashboard.tsx`, `klaviyo-dashboard-support.ts`). |
| KLAVIYO-INTEGRATIONS-27 | CLOSED | New finding. Klaviyo was filed under "Coming soon"; the design puts it in the live grid as the sixth card. It is now the sixth live card with the design's description and status, and no button (see 28). |
| KLAVIYO-INTEGRATIONS-28 | CLOSED (credential-gated) | The start/callback pair exists: `app/api/oauth/klaviyo/start/route.ts` (requireBusinessAccess `collaborator`, base64url state mirrored into an httpOnly cookie, PKCE S256 verifier in a second cookie) and `app/api/oauth/klaviyo/callback/route.ts` (state/cookie compare, re-authorised at `collaborator`, code+verifier exchanged, `upsertIntegration({provider:"klaviyo"})`, first import attempted and reported truthfully via `syncScheduled`). Both are covered by `route.test.ts` suites, and `oauth/klaviyo/start` has been removed from `INTENTIONALLY_PUBLIC` in `app/api/route-authority.test.ts` because it now enforces its own authority. The start route still answers 501 — naming `KLAVIYO_CLIENT_ID` / `KLAVIYO_CLIENT_SECRET` and the redirect URI — until the owner creates a Klaviyo OAuth app, and `/api/klaviyo/status` reports `connectable:false` in that state so the card keeps showing no button rather than one that fails. |
| KLAVIYO-INTEGRATIONS-29 | CLOSED | `app/api/klaviyo/flows/route.ts` (requireBusinessAccess `minRole:"guest"`, copied from `app/api/search-console/analytics/route.ts`) serves the five columns out of the additive `klaviyo_flow_metrics` table created in `lib/migrations.ts` and filled by `lib/klaviyo/sync.ts` behind `assertSyncLaneEnabled("source_ingest")` + `assertSyncGrowthBoundary`. `klaviyo-client.tsx` fetches it. Every metric column is nullable with no default, so an unreported statistic stays an em-dash: `klaviyo-client.test.tsx` renders `["Win-back 60d","Draft","—","—","—"]` from a served row and `["—","—","—","—","—"]` from `flows:null`. |
| KLAVIYO-INTEGRATIONS-30 | BLOCKED | The roadmap cards' ETA line (design 4349-4353, e.g. "decision contracts in design · Q4 2026") has no source; a hardcoded quarter would be a commitment the product has not made. Renders "—" in the exact `.soonEta` slot. **Contract to unblock:** a roadmap/ETA source for non-live providers. |
| KLAVIYO-INTEGRATIONS-31 | CLOSED (one signal stays em-dashed) | `GET /api/google-analytics/status` and `GET /api/google-search-console/status` now serve the first-import facts, both guarded by `requireBusinessAccess` at `minRole: "guest"` — the shape `/api/shopify/status`, `/api/integrations/status` and `/api/google-search-console/sites` already use. Both cards render the design's block while their warmer is genuinely in flight and never after. Three of the four signals are real; `backfillFraction` stays null by contract because neither provider has one to give. See 31 below. |
| KLAVIYO-INTEGRATIONS-F1 | CLOSED | Adversarial-review finding, blocking. The first-sync block fabricated a first import for long-connected providers. Existence is now evidence-bound and the "Connecting" pill can no longer cover a fault. See F1 below. |
| KLAVIYO-INTEGRATIONS-F2 | CLOSED | Adversarial-review finding. Invented static copy "no authorization flow available yet" replaced by the design's own non-connected line, "no data pulled yet" (model 4342). See F2 below. |
| KLAVIYO-INTEGRATIONS-F3 | CLOSED | Adversarial-review finding. Three fabricated roadmap descriptions deleted from a shipped constant. See F3 below. |
| KLAVIYO-INTEGRATIONS-F4 | CLOSED (recorded extension) | Adversarial-review finding. "Degraded" / "Needs setup" / "Action required" are captions the design does not define; kept deliberately and recorded, with the branches now reachable. See F4 below. |
| KLAVIYO-INTEGRATIONS-F5 | CLOSED | Adversarial-review finding. The rail's Klaviyo entry could appear on route evidence alone; it is now gated purely on the connection being live, per design 3284. See F5 below. |
| KLAVIYO-INTEGRATIONS-F6 | CLOSED (partly recorded) | Adversarial-review finding, cosmetic. `.cardName` truncation removed so the name wraps as the design lets it; the added `flex-shrink: 0` is kept and recorded. See F6 below. |

### KLAVIYO-INTEGRATIONS-31 · CLOSED — GA4 and Search Console now serve first-import status

- **What was missing:** `app/api/google-analytics/` held only `properties` and `select-property`; `app/api/google-search-console/` only `sites` and `select-site`; `app/api/search-console/` only `sites` and `analytics`. Nothing served the four `FirstSyncSignals` facts, so `buildIntegrationsExactModel` passed `null` for both providers and their cards could never show the block.
- **Where the facts actually live.** Both providers' importer is the report warmer, and it is the only thing that writes durable state for them:
  - `lib/sync/ga4-sync.ts` upserts one `provider_sync_jobs` row per window (`provider='ga4'`, `report_type='ga4_overview'`) — `running` on entry, `done`/`failed` on exit — and warms `provider_reporting_snapshots` rows under `provider='ga4'`.
  - `lib/sync/search-console-sync.ts` does the same with `provider='search_console'`, `report_type='seo_overview'`, writing `seo_results_cache` rows.
  - Both are driven by `app/api/sync/cron/route.ts`.
- **What is wired, fact by fact:**
  | Signal | GA4 source | Search Console source |
  | --- | --- | --- |
  | `connected` | the `ga4` connection row is in `connected` status | the `search_console` connection row is in `connected` status |
  | `importing` | `state === "syncing"`: the newest `ga4_overview` job row is `running` and inside the 15-minute boundary `lib/admin-operations-health.ts` already uses to call such a job stuck (its own query selects `provider IN ('google_ads','ga4','search_console')`) | same, on the newest `seo_overview` job row |
  | `entitiesReady` | a property is selected — `metadata.ga4PropertyId` / `metadata.propertyResourceName` / `provider_account_id` | a site is selected — `metadata.siteUrl` / `provider_account_id`, the same read `resolveSearchConsoleContext` does |
  | `snapshotReady` | a `provider_reporting_snapshots` row exists for `provider='ga4'`, `report_type='ga4_analytics_overview'` — exactly the row `app/api/analytics/overview/route.ts` reads before it will call Google | a `seo_results_cache` row exists with `cache_type='overview'` — exactly the row `app/api/seo/overview/route.ts` reads first |
  | `backfillFraction` | **null — em-dashed, see below** | **null — em-dashed, see below** |
- **The one signal that stays em-dashed, and why.** Neither provider has a backfill queue to observe. Their historical data already exists at Google; the "first import" is one Data API / Search Analytics call per window, and the warmer either stores that window whole or stores nothing. The only two observable points are 0% and the instant the row exists — at which instant `snapshotReady` is already true and the block is gone by the design's own rule. The available substitute (counting landed report types or cache types) is a share of a *report set*, not a share of the 28-day window, so putting it under the design's "Backfill 28 days" caption would state a number the data does not support. `backfillPercent` is therefore `null`, and `resolveFirstSyncPercent` parks the bar at the start of the backfill stage with all four steps in their designed states — the identical treatment Shopify's unfinished backfill already gets (`shopifyFirstSyncSignals` returns `null` for the same reason). The field is typed `number | null` so a genuine day-level source can fill it without a shape change.
- **The five-condition rule is honoured, not bypassed.** `ga4FirstSyncSignals` / `searchConsoleFirstSyncSignals` feed the same `buildFirstSyncModel`. Only `syncing` is in `IMPORTING_PROVIDER_STATES`; `connected_no_property` / `connected_no_site` wait on the operator, `awaiting_first_sync` has nothing in flight, `first_sync_stalled` has stopped moving, `action_required` is broken, `ready` has landed. A long-connected GA4 property still fails condition 5 (`connectedAt` older than 28 days), so it can never render "first import running".
- **Search Console's borrowed authority is reported.** Search Console reads on the `google` connection's credential (`lib/search-console.ts` requires a connected `google` row carrying `webmasters.readonly`). The status response carries `googleAuthority: { connected, hasSearchConsoleScope }` and resolves to `action_required` when either is false — so a Search Console card whose Google principal was revoked names the fault instead of painting a bar.
- **No migration.** Every table read (`provider_connections`/`integration_credentials` via `getIntegrationMetadata`, `provider_sync_jobs`, `provider_reporting_snapshots`, `seo_results_cache`) already exists in `lib/migrations.ts`; each read is guarded by `getDbSchemaReadiness` and degrades to "no evidence", which yields no block.
- **Proof:** `lib/google-analytics-status.test.ts` (10 cases) and `lib/search-console-status.test.ts` (10) walk every state including the stuck boundary and the broken-scope case; `lib/provider-report-sync-evidence.test.ts` pins the 15-minute boundary at, inside and outside; `app/api/google-analytics/status/route.test.ts` and `app/api/google-search-console/status/route.test.ts` assert the 400-before-auth, the `minRole: "guest"` shape, the refusal path and `Cache-Control: no-store`; `integrations-exact-adapter.test.ts` asserts 35% for both providers, no block for any non-importing state, and no block for a months-old GA4 property; `app/(dashboard)/integrations/page.test.tsx` renders both cards' blocks end to end and asserts their absence for a long-connected property and for a broken Search Console authority.

### F1 · BLOCKING · WRONG — The "First sync" block fabricated a first import for long-connected providers

- **Design:** 2819 — "Sync progress appears once: while a new source runs its first import." Model 4332-4348 drives `syncing` from `st.sync[b.id]`, a per-source entry that exists only between `startSync` (3214-3226) and the moment that source's snapshot lands.
- **Code (before):** `integrations-exact-adapter.ts` set `snapshotReady: status.state === "ready"` and treated every other connected state as an import in flight. Both provider unions carry long-lived non-"ready" states while connected (`lib/google-ads/status-types.ts:136-145` — paused | partial | advisor_not_ready | stale | action_required; `lib/meta/status-types.ts:224-232` — partial | stale | paused | action_required). A Google account connected months ago whose token was later revoked reported `state: "action_required"` with `historicalProgress.percent: 100`, and the card rendered "First sync — 82%" under a "Connecting" pill with the meta line "first import running — nothing shows in the app until the snapshot lands". Every word of that was false. `resolveStatus` made it worse by short-circuiting on `syncing` before it read `view.status`, so the fault was never named.
- **The rule now in force — a provider is running its FIRST import only when all five hold:**
  1. the stored connection is healthy — `ProviderViewState.isConnected` (an expired or errored connection is not a new source, whatever its status endpoint still says);
  2. the provider's status endpoint says `connected`;
  3. it has **not** reported a servable snapshot (`state === "ready"`, or Meta's `coreReadiness.complete`);
  4. its own state says work is in flight — `syncing` or `partial` only. `paused` was halted, `stale` fell behind *after* landing, `action_required` is broken, `advisor_not_ready` is a downstream verdict and `connected_no_assignment` waits on the operator: none of those may paint a bar;
  5. the connection itself is younger than `FIRST_IMPORT_MAX_CONNECTION_AGE_MS` = 28 days — the window the design's own third stage ("Backfill 28 days") exists to land. A connection older than the window its first import had to cover is not still running it. `connectedAt` comes from `ProviderViewState.connectedAt`; unknown or unparseable yields **no** block, because nothing then proves the import is the first.
- **Fix (b):** `resolveStatus` now reads `action_required` and `degraded` **before** the `syncing` short-circuit, so a broken or degraded provider can never read "Connecting" — which also makes those branches reachable (see F4).
- **Proof:** `integrations-exact-adapter.test.ts` walks Meta, Google and Shopify through `stale`, `paused` and `action_required` asserting `firstSync === null`, `syncing === false` and the connected caption; asserts a broken connection reads "Action required" and a degraded one "Degraded" even while the provider state says `syncing`; asserts a two-hour-old importing Google connection still renders 82%. `isFirstImportConnection` is unit-tested at, inside and outside the window, plus unknown and unparseable dates. `app/(dashboard)/integrations/page.test.tsx` renders the exact reported failure (expired connection, `action_required`, `historicalProgress.percent: 100`) and asserts no block, no "first import running" line and an "Action required" pill.

### F2 · MEDIUM · INVENTED COPY — Non-connected meta line said "no authorization flow available yet"

- **Design:** model 4342 — the non-connected branch is the literal `'no data pulled yet'`. The string "no authorization flow available yet" appears nowhere in the reference.
- **Code (before):** `INTEGRATIONS_META_NO_AUTH_FLOW` in `integrations-exact-adapter.ts:80`, applied to any provider without a working authorization route (only Klaviyo today).
- **Fix:** constant deleted; every non-connected card, Klaviyo included, uses `INTEGRATIONS_META_NOT_CONNECTED = "no data pulled yet"`. The missing button already carries the fact that there is nothing to click, exactly as the design's own `showBtn` conditional does. Asserted in both the adapter test and the route test.

### F3 · MEDIUM · INVENTED COPY — Fabricated descriptions for the three roadmap providers

- **Design:** `soonIntegrations` (4349-4353) carries `name`, `logo` and `eta` — no description — and the roadmap card markup (2870-2877) has no slot for one.
- **Code (before):** `INTEGRATIONS_PROVIDER_DESCRIPTIONS` invented "Campaign performance from TikTok Ads.", "... from Pinterest Ads.", "... from Snapchat Ads." — dead copy `SoonCard` never rendered.
- **Fix:** the three entries are deleted and the record is typed `Record<IntegrationsLiveProvider, string>`, so the type system now refuses a description for a provider the design gives none to.

### F4 · LOW · EXTRA (deliberate, recorded) — Status vocabulary beyond the design's three captions

- **Design:** model 4341 defines three captions — `Connecting` / `Connected` (`Connected · Beta` for Klaviyo) / `Not connected` — because its fixture has three states: syncing, connected, not connected.
- **Code:** `resolveStatus` also returns **"Degraded"**, **"Needs setup"** and **"Action required"**.
- **Decision: keep, deliberately.** This product's connections have states the reference fixture does not model — a revoked token, a stale discovery cache, a connection with no accounts assigned. Collapsing those into "Connected" would state something false about the data behind the card, and collapsing them into "Not connected" would state something false about the connection. Each caption is the honest name for a state the store already derives (`ProviderPresentationStatus` in `store/integrations-store.ts`), and each keeps the design's pill geometry and its two extra tones (`attention` reuses the design's warning pair, `connecting` its info pair).
- **Reachability:** before F1(b) the `action_required` and `degraded` branches were unreachable whenever a first import was believed to be running, which is precisely when a fault most needs naming. They are now read first, and both branches are covered by tests.

### F5 · MEDIUM · WRONG — The rail could put Klaviyo in the sidebar on route evidence alone

- **Design:** 3284 — `navPlatforms` includes Klaviyo only when `st.klaviyoOn`, which the reference sets when Klaviyo's first sync completes (3224). 2864 and 4321 both say a source "stays out of the sidebar until the integration is live".
- **Code (before):** `components/layout/v2/app-rail.tsx` computed `showKlaviyo: klaviyoSnapshotReady || pathname.startsWith("/platforms/klaviyo")`. The first half is correct and was already wired to the stored connection; the second half put the entry in the rail purely because the user was standing on the Klaviyo route — which, now that this batch made `/platforms/klaviyo`, `/app/klaviyo` and `/c/{businessId}/klaviyo` resolve, meant landing on the em-dash table put a permanent-looking entry in the sidebar for a source that is not live.
- **Fix:** the pathname clause is deleted; `showKlaviyo` is now exactly `connection.status === "connected" && connection.lastSyncAt`. `components/layout/nav-items.ts` keeps its Klaviyo registry row — that row is metadata (name, BETA badge, logo), not a rail entry; the gate lives in `getRailModel`, and a comment now says so. Covered by three `app-rail.test.tsx` cases: out while on the Klaviyo route with no connection, out while connected but never synced, in once a sync has landed.

### F6 · LOW · GEOMETRY — Card name truncation removed; `flex-shrink: 0` kept and recorded

- **Design:** 2816 — the name span declares only `font-size:14.5px;font-weight:600;color:#0E1526`. No `overflow`, no `text-overflow`, no `white-space`: a long name wraps and the card grows. Only the footer's meta line ellipsizes (2848), and that one the design asks for explicitly.
- **Code (before):** `.cardName` added `min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap`, so "Search Console" next to an "Action required" pill in a 300px column lost characters the design always shows.
- **Fix:** the four declarations are removed and the name wraps again.
- **Kept, deliberately:** `flex-shrink: 0` on `.logoTile`, `.statusPill`, `.cardButton` and `.soonLogoTile`. These are not in the reference's inline styles, but they preserve pixel geometry the reference *does* declare (34px tile, `3px 10px` pill, 30px button, 32px roadmap tile) once the name wraps in the narrowest grid column. Removing them would let a browser squash those boxes below the sizes the design pins, which would be a larger divergence than the declaration itself.
...
| INSIGHTS-A-37 | CLOSED | GA4 money now carries its unit end to end. `fetchGA4PropertyMetadata` parses `Property.currencyCode` out of the Admin `properties/{id}` read it already made for `timeZone` (`lib/google-analytics-accounts.ts`), `select-property` persists it as `ga4PropertyCurrency` beside `ga4PropertyTimeZone`, `resolveGa4AnalyticsContext` / `getGA4TokenAndProperty` read it back as `propertyCurrency` / `currencyCode`, and all six endpoints serve it: `/api/analytics/overview`, `/products`, `/audience`, `/demographics`, `/cohorts` and `/api/geo/traffic-sources` each return `currency`. Both adapters format with `formatMoneyIso`, so a EUR property reads `€326.4K` and a TRY property `TRY 326.4K`. No migration was needed — the key lives in the existing `integration_credentials.metadata` JSONB. A property selected before the code was stored serves `null` and renders `—`, never a guessed `$`; re-selecting the property in Integrations fills it in, because no read path may write (`lib/read-path-write-reachability.test.ts`). Covered by `lib/google-analytics-accounts.test.ts`, `lib/google-analytics-reporting.test.ts`, `lib/ga4-user-facing-reports.test.ts`, `lib/analytics-overview.test.ts`, `app/api/google-analytics/select-property/route.test.ts`, `app/api/geo/traffic-sources/route.test.ts` and both exact-adapter tests. |
...
- **Fix:** Return the property's ISO-4217 currency from the six endpoints that serve GA4 money — the five analytics endpoints and `/api/geo/traffic-sources` — then format with `formatMoneyIso` and render `—` when the code is absent. **Widened (batch 11):** the AI Sources tab's Revenue column reads GA4 `purchaseRevenue` through `/api/geo/traffic-sources` and prints it with the same hardcoded `$`, so that endpoint is now named in the contract instead of being an unlisted sixth instance. **Correction (batch 11):** this block previously said the currency "is on the Admin API property record `getGA4TokenAndProperty` already resolves". It is not. `getGA4TokenAndProperty` (`lib/google-analytics-reporting.ts:374-391`) makes no Admin API call; it returns `accessToken` / `propertyId` / `propertyName` from stored integration metadata. The only `properties/{id}` GET in the repo is `fetchGA4PropertyMetadata` (`lib/google-analytics-accounts.ts:185-218`), which parses `timeZone` and nothing else, and it is invoked once at property selection. So the fix requires a new Admin field read plus metadata persistence before any endpoint can serve the code — the BLOCKED status is right, the effort estimate implied by the old wording was not.
- **Closed (batch c1):** the whole path is wired. `Property.currencyCode` is parsed in `fetchGA4PropertyMetadata` (same Admin request that already read `timeZone`), persisted at selection as `metadata.ga4PropertyCurrency`, read back by `resolveGa4AnalyticsContext` as `propertyCurrency`, and served as `currency` by all six endpoints. `formatMoney` in `components/analytics/insights-analytics-exact-adapter.ts` and the Revenue cell in `components/geo/insights-geo-exact-adapter.ts` both call `formatMoneyIso` with the code that came with their own payload — one endpoint's code never labels another's money. **Deliberately still `—`:** a property selected before this change has no stored code, and the missing value was chosen over a bare unlabelled number because these tables sit beside Meta and Google money that *is* currency-marked, where an unmarked figure reads as the reader's own currency. One re-selection of the property in Integrations stores the code; nothing on a read path writes it, which `lib/read-path-write-reachability.test.ts` exists to enforce.
...
`InsightsSeoScreen` is also the product's **only** caller of
`POST /api/seo/ai-analysis`. There is no cron and no worker: if this screen does
not offer the run, no monthly analysis can ever be produced and the Monthly AI
and Actions tabs are unfillable. The control is described in INSIGHTS-B-42 —
it takes the design's cadence-chip slot and appears only in the pre-generation
...
original pre-resolution audit evidence; this table is the current status.
Findings 38–41 are new and were raised by this batch's own read. Findings
**42–49** are newer still: they were raised by an adversarial verification pass
over this batch's first commit, which found no wrong caption, column, chip or
palette value on either surface but did find that the batch had traded away
truthfulness in six places and orphaned a backend capability in a seventh.
Every one of them is a defect this batch introduced, not an inherited one.
...
| INSIGHTS-B-41 | CLOSED | The contract this row asked for is built. `SeoFindingSeverity` gains `passed`; `runPageChecks` (ex-`buildFindingsForPage`) now reports the checks it actually evaluated alongside its findings, and `buildPassedFindings` emits one passing finding per audited page that carries no other finding, ran at least one check, and was not reported excluded by URL Inspection. The finding names the subset it cleared — a Homepage exempt from the canonical check, or a URL Google was never asked about, is reported as "Pages cleared every technical check that ran" with the check list and an explicit "URL Inspection did not run for these URLs", never as healthy; only a `PASS` verdict earns the "including URL Inspection" title. `summarizeFindings` counts `passed` as a fourth distinct-page bucket and the adapter's `seoPassedPageCount` now reads `summary.passed` — the subtraction is deleted, not kept as a second source of the same number — so a payload cached before the severity existed renders the em-dash. The chip reads `Passed` in the design's C.pos green; `Opportunity`, a severity the design has no chip for, moves to C.info so only one chip in the list is green. Proof: `lib/seo/findings.test.ts` (10 tests, incl. the excluded-but-clean page that never passes and `passed + flagged === auditedPageCount`), plus the tone and caption pins in `insights-seo-exact-adapter.test.ts` and `InsightsSeoExact.test.tsx`. |
| INSIGHTS-B-42 | CLOSED | **Raised by verification.** This batch orphaned the Monthly AI generator. `HEAD~1:app/(dashboard)/insights/seo/legacy-page.tsx:119-137` defined a `POST /api/seo/ai-analysis` fetcher and surfaced it at `:394` and `:500`; the replacement `InsightsSeoScreen` issued only GETs, and `grep -rn generateSeoMonthlyAiAnalysis` then returned exactly two sites — the definition (`lib/seo/run-monthly-ai-analysis.ts:458`) and the uncalled POST handler (`app/api/seo/ai-analysis/route.ts:194`). No cron, no worker, no other client. The Monthly AI and Actions tabs are fed only by the stored analysis, so any business that had never generated one would have shown two permanently em-dash tabs that nothing in the product could ever fill. **Decision: the trigger is restored, not scheduled and not filed as blocked.** Scheduling would have invented a cadence the product does not own (the route is guarded at `collaborator` and spends an OpenAI call), and filing it as BLOCKED would have left a live capability unreachable for the sake of a state the design never draws. **Where it lives:** the design draws only the post-generation state, and its cadence chip (L1979) is `cursor:not-allowed`, so the chip is not the trigger. `seoMonthlyGenerateControl` returns a control *only* when the served status is `not_generated` or `failed` **and** the server's own `canGenerate` is `true`; that control takes the cadence chip's exact slot, border, radius, padding and type (`.monthlyGenerate`, declared outside the reference marker and labelled as not-from-the-design). In the state the design draws, the design's inert chip renders unchanged and the tab has no buttons but its six sub-tabs — both pinned in `InsightsSeoExact.test.tsx`. `requireBusinessAccess` on the POST route is untouched. |
| INSIGHTS-B-43 | CLOSED | **Raised by verification.** The Actions grouping heuristic was invented and the design's own data refuted it. `seoActionGroupFor` tested `effort === "low"` first, but script L4010 files "Internal links from the two top guides to money pages" — explicitly `low effort · week 2` — under **Strategic**, so the shipped rule would have moved that exact item into Quick wins. Reading all five items at L4003-4012 with the impact and effort their copy states gives a rule that reproduces every one of the design's placements: **impact leads, effort breaks the tie** — a top-impact item is Strategic whatever it costs, a cheap item that is not top-impact is a Quick win, everything else is Supporting. The order is inverted in the adapter and the design's five items are the test fixture, with the counter-example asserted by name. |
| INSIGHTS-B-44 | CLOSED | **Raised by verification.** The `Passed` card broke the design's four-card arithmetic. `seoPassedPageCount` subtracted the distinct paths carrying **any** finding, `opportunity` included, so a page whose only finding was an opportunity appeared on none of the four cards and a reader doing the design's own subtraction (148 − 5 − 12 = 131, script L4013) got a different answer than the card showed. Only the critical ∪ warning paths are subtracted now — the same distinct-page population `summarizeFindings` (`lib/seo/findings.ts:1233-1250`) counts for the other two cards. Pinned by a fixture with disjoint severities asserting `audited − critical − warnings === passed`, and by a fixture whose opportunity-only page counts as passed. **Disclosed:** the design's subtraction assumes the two severity sets are disjoint. A page carrying both a critical and a warning finding is counted once in the union and twice by a reader subtracting the two cards, so `Passed` will read one higher than that subtraction for each such page. Counting the union is the honest choice — the alternative double-counts a page — and the discrepancy exists in the design's own arithmetic, not in the data. **Superseded by INSIGHTS-B-41:** the subtraction is gone entirely. `Passed` is now a served per-page verdict (`summary.passed`), which removes both the disjointness assumption disclosed above and the residual dishonesty this row settled for — an opportunity-only page is no longer silently counted as passing. What the four cards reconcile to is the builder's own invariant, `Pages audited = Passed + pages carrying at least one critical, warning or opportunity finding`; on data shaped like the design's, that is exactly 148 − 5 − 12 = 131, pinned by `DESIGN_SHAPED_FINDINGS`. |
| INSIGHTS-B-45 | CLOSED | **Raised by verification.** The Query Intelligence footnote no longer described its own table. L2248 says "counts reflect the full 220-query dataset, table shows the top rows by GEO score", but `/api/geo/queries` returns every scored row (`rowLimit` 500, no slice) and `buildGeoQueries` applied the filter and nothing else, so the table showed *all* rows of the filter in the route's own order (`route.ts:164-171`: breakout/rising first, then `isAiStyle`, then `geoScore`) — not the top rows, and not by GEO score. The sentence is now true of the code rather than restated over it: the table is re-sorted by `geoScore` descending (a stable sort, so ties keep the route's order) and bounded to `GEO_QUERY_TABLE_ROWS = 8`, which is what the design's own table renders (8 of 220, and its `GQ` seed is already in score order: 78, 74, 69, 66, 61, 52, 41, 38). |
| INSIGHTS-B-46 | CLOSED | Closed with A-37. `/api/geo/traffic-sources` now returns the property's `currency` beside the rows, and `buildGeoSources` renders the AI Sources Revenue column with `formatMoneyIso(revenue, { currency })` — `€1.9K` for a EUR property, `TRY 1.9K` for a TRY one, `—` when the property carries no code, and never a hardcoded `$`. Asserted in `components/geo/insights-geo-exact-adapter.test.ts` and `app/api/geo/traffic-sources/route.test.ts`. |
| INSIGHTS-B-47 | CLOSED | **Raised by verification.** GEO topic chips were silently truncated. `buildGeoTopics` capped them at `.slice(0, 3)`, but L2274-2276 renders every entry of `t.chips` and the design's own fourth topic carries 2 (script L4082) — `hint-placeholder-count="3"` is a preview hint, not a content cap. The cap is removed; the adapter test asserts a four-chip cluster renders four and a one-chip cluster renders one. |
| INSIGHTS-B-48 | CLOSED | **Raised by verification.** Two different numbers stood for the same design fact on one surface. The Overview tab's intent band reads `kpis.aiStyleQueryCount` / `kpis.totalQueryCount` from `/api/geo/overview` (`route.ts:301`), while the Queries tab derived both its filter-pill counts and the footnote's dataset size from the `/api/geo/queries` row count — a different fetch with a different AI-style classifier — so "All queries · N" and the footnote's N could disagree with the band's own N. `buildGeoFilters` now takes the overview and reads `All queries` from `totalQueryCount` and `AI intent` from `aiStyleQueryCount`, and `queryDatasetSize` is `totalQueryCount` (em-dash when unserved), never the number of rows on screen. Asserted by a test that compares the pills against `buildGeoIntentBand`'s own output. **Disclosed:** `High impressions`, `Weak CTR` and `Rising ↑` have no served count and are still counted over the rows `/api/geo/queries` returns — the same 500-row Search Console population and window `totalQueryCount` is measured from, but classified by `classifyQuery` rather than the overview's `scoreQueryIntent`. |
| INSIGHTS-B-49 | CLOSED | **Raised by verification.** A deleted guard was not re-homed. `components/zero-base/analytics/analytics-source-truth.test.tsx` (187 lines, five WP-21 assertions) was deleted with `AnalyticsSourceClient` / `SeoClient`. The subject is genuinely gone, but the defect it guarded — a surface reading connection state off its own report payload and drawing a healthy provider as down — is not. `components/insights/insights-source-truth.test.tsx` re-homes all five against `InsightsChrome`, the surface that renders that posture now: a connected, serving provider is never drawn as down; "down" is claimed only when the authority says disconnected; the two source chips are exactly GA4 and Search Console, so no provider that supplied none of the numbers gets a panel beside them; and a Search Console read reports GA4 on GA4's own terms. **One assertion needed a code change to re-home honestly:** "reports an unreadable authority as unknown rather than down" did not hold — before the manifest is read the store holds defaults that derive as `disconnected`, so both chips asserted "not connected" for a fully connected business. `InsightsChrome` now passes `authorityRead` and the chips read `reading status` until the bootstrap completes. **Residual gap, disclosed:** `bootstrapStatusByBusiness` is only `idle | loading | ready` (`store/integrations-store.ts:181`) and `useBusinessIntegrationsBootstrap` swallows a failed manifest read, finishing as `ready` with zero connections. A *failed* authority read is therefore still indistinguishable from a genuine disconnection. **Contract required:** the store needs a `failed` bootstrap status, after which the unknown state can survive the failure instead of collapsing into "not connected". |
...
Canonical source read in full for this batch: Reports markup lines
**2324–2617** and their complete model bindings at lines **3178–3180** and
**4105–4212** of `Adsecute Dashboard v2.dc.html` at SHA-256
`d65c0117871aa392fb2f93e79d02540f6538be6a00b1d2ecea03bdd9f8432193`
(read locally with `sed`; every API read of that file truncates at line 2225).

`ReportsExact` is now the single presentation surface for the whole screen —
the My reports, Templates and Builder tabs, and the builder's palette / canvas
/ inspector columns. `reports-exact-container.tsx` is the only fetching layer;
it reads the business's saved reports from `GET /api/reports`, renders the
working document through `POST /api/reports/render` so canvas blocks carry
measured figures, and autosaves through `POST`/`PATCH /api/reports`. Six route
families converge on it: `/reports`, `/reports/new`,
`/reports/[reportId]/edit`, `/c/[businessId]/reports`,
`/c/[businessId]/reports/new`, `/c/[businessId]/reports/[reportId]/edit` (and
therefore `/app/reports*`). `components/reports/report-builder.tsx` — the 93KB
builder every EXTRA finding below described — is deleted;
`report-builder-page.tsx` is now the rendered-report view only.

Backend work landed for this batch, rather than deferred to `—`:

- `CustomReportWidgetType` gained the design's block kinds (`kpirow`, `donut`,
  `funnel`, `heat`, `ai`, `brief`); `section` stays in the union because stored
  documents contain it, and it renders through the design's own text fallback.
- Blocks now store `size` (S/M/L) and keep `colSpan` in step, so the print and
  share surfaces keep laying documents out unchanged.
- `renderCustomReport` builds two of the new kinds for real: `kpirow` from the
  blended summary pins and `donut` from the channel-attribution rows, both with
  `—` for anything unmeasured. Proof: `lib/custom-report-renderer-v2-blocks.test.ts`.
- `resolveDateRangePreset` gained `28` and `this_month` so the toolbar's ranges
  resolve to real windows instead of being relabelled 30-day ones.

`CLOSED` means the source-level DOM, geometry, route, data or truthfulness
divergence is removed and covered by focused tests. It does **not** claim a
...
| REPORTS-02 | CLOSED | The canvas opens on the design's page header (accent square, name, client · window · Page 1); no Canvas heading, description textarea or template chip exists. Test: "draws the report page header with the accent square and page meta". |
| REPORTS-03 | CLOSED | Blocks carry no resize handles; width comes from the size chip and the inspector's Width segments. |
| REPORTS-04 | CLOSED | The block inspector is Title, Metric, Source, Date range, Width, compare, Remove block and the footnote. Test: "has no table, breakdown, body or subtitle sections". |
| REPORTS-05 | CLOSED | No back control and no quick-add strip; blocks are added from the palette and deselected by clicking the block again. |
| REPORTS-06 | CLOSED | Both tabs render the bare article/grid plus a footnote; `ProductSection` is gone from this screen. |
| REPORTS-07 | CLOSED | The toolbar is name, range, compare pill, spacer, counter, Preview, Save & schedule. Test: "has no back control and no actions dropdown". |
| REPORTS-08 | CLOSED (geometry) / BLOCKED (contract) | The Report settings state renders with Client, Schedule, Recipients and the Live share link toggle at the design's geometry. Client names the real active business. Schedule, Recipients and the live-link toggle are inert and read `—`: `custom_reports` has no schedule, recipient or live-link column and nothing in the repo sends a report. **Contract needed:** `custom_reports.schedule jsonb`, `custom_reports.recipients text[]`, `custom_reports.live_share_token`, plus a scheduler that renders and sends on cadence. |
| REPORTS-09 | CLOSED | Page header renders with the 2px `#0B1020` rule, the 10px `#2F6BFF` square and the real client and rendered window. |
| REPORTS-10 | CLOSED | Every block has the ⠿ handle, the mono uppercase title, the S/M/L cycle chip, the ✕ remove and the mono provenance line derived from its own source and the report window. Test: "gives every block a drag handle, a size chip, a remove control and provenance". |
| REPORTS-11 | CLOSED | The palette has the design's four groups and all thirteen blocks with the per-group icon colours. Test: "groups the thirteen blocks under the design's four headings". |
| REPORTS-12 | CLOSED | Source is a labelled select of the design's six options, writing `dataSource` on the block. The logo strip is gone. |
| REPORTS-13 | CLOSED | Width is the ⅓ / ½ / Full segmented control writing the block's size. |
| REPORTS-14 | CLOSED | The compare checkbox and the red-outline Remove block button are both present and wired. |
| REPORTS-15 | CLOSED | The panel head is the mono "Block settings" eyebrow plus the block-kind chip from `BUILDER_KIND_NAMES`. |
| REPORTS-16 | CLOSED (geometry) / BLOCKED (contract) | The per-block Date range select renders with Report range / Last 7 days / Last 28 days at the design's geometry, disabled. Every block resolves on the report window; the renderer caches its sources per report range. **Contract needed:** per-widget window resolution in `renderCustomReport` and a `dateRange` field on `CustomReportWidgetDefinition`. |
| REPORTS-17 | CLOSED | Template cards carry the cadence, the numbered 01–05 contents derived from the template's own blocks, the meta footer derived from block count + providers, and both Preview and Use template. |
| REPORTS-18 | CLOSED | The toolbar has Preview and the mono counter. The counter says "autosaved" only after a save has actually landed — `saving…` / `not saved` otherwise. Test: "only says autosaved once a save has actually landed". |
| REPORTS-19 | CLOSED | The canvas is `flex-wrap` at 10px gap with 33.333% / 50% / 100% block widths; drop-on-block inserts before the hovered block and drop-on-canvas appends. Test: "lays blocks out on the design's S/M/L flow widths". |
| REPORTS-20 | CLOSED | Palette labels and source tags match the design exactly; there is no Section block. Test: "labels and tags each block exactly as the design does". |
| REPORTS-21 | CLOSED | Metric is one select of the design's seven options; the search box, multi-row list and "+ Metric" button are gone. |
| REPORTS-22 | CLOSED | Row actions are Open in builder, Duplicate, Share link, PDF, in that order; Delete is removed. Share link creates a real snapshot through `POST /api/reports/[id]/share` and copies the URL; PDF opens the existing print surface. |
| REPORTS-23 | CLOSED (caption) / BLOCKED (schedule) | The primary action is "Save & schedule" at `#0B1020`. It saves; there is no scheduler, so the "& schedule" half is unbacked — same contract as REPORTS-08. |
| REPORTS-24 | CLOSED (three of four) / BLOCKED (custom) | The range select offers Last 28 days / Last 7 days / This month / Custom range…, defaulting to 28. The first three resolve to real windows in `resolveDateRangePreset`. Custom range… is rendered but `disabled`: `CustomReportDocument` has no custom window and the design draws no picker. **Contract needed:** `customStart`/`customEnd` on the document. |
| REPORTS-25 | CLOSED | The builder is a tab of the Reports screen. No `min-h-screen`, no page background, no edge-to-edge bar; the toolbar is a 14px-radius article and the three columns sit below it at 12px gap. Test: "never introduces a page shell inside the workspace column". |
| REPORTS-26 | CLOSED | No search input and no sort select. Test: "has no search box and no sort control". |
| REPORTS-27 | CLOSED | An empty builder shows only the dashed drop zone with the design's copy. Test: "carries one dashed drop zone and no onboarding panel". |
| REPORTS-28 | CLOSED | The card header is the category chip plus the cadence; providers appear only in the footer meta string. |
| REPORTS-29 | CLOSED | The header is eyebrow + h1 only; the export note sits at the right end of the tab row. |
| REPORTS-30 | CLOSED | The h1 is Space Grotesk 26px/700 at −0.02em and the eyebrow 11px at 0.12em, set in `ReportsExact.module.css` rather than by editing the shared shell rule. |
| REPORTS-31 | CLOSED (geometry) / BLOCKED (contract) | The status badge renders at the design's geometry beside the name and reads `—`, because no send state is stored against a report. Same contract as REPORTS-08. Test: "never claims a send state, because none is stored against a report". |
| REPORTS-32 | CLOSED | Both mono footnotes render under the list and under the template grid. |
| REPORTS-33 | CLOSED | Comparison is the `#EAF0FF` pill; per-block comparison comes from the inspector checkbox. Test: "shows comparison as a pill, not a control". |
| REPORTS-34 | CLOSED | The rows are one 14px-radius article divided by 1px `#F3F5F9` rules, each laid out as 86px mosaic thumbnail / text column / action cluster on one line. |
| REPORTS-35 | CLOSED | The widget-count chip is gone; the block count is folded into the row's mono meta line. |

Found during this batch and not in the list above:

### REPORTS-36 · HIGH · WRONG — Builder canvas figures were prototype seed values, never rendered data

- **Design:** l.2445-2447 `{{ w.big }}` / `{{ w.delta }}`; data-model.js `kpiSamples = [['$118,220','+8.1%'], ['4.09','+3.6%'], …]` and `minis: [{k:'Spend',v:'$118.2k'}, …]` — the reference fills every block from a fixed sample array, so a canvas ported literally shows Aurora Supply Co.'s numbers for every business.
- **Code (before):** the builder never called any render endpoint while editing; there was no path from a block to a measured figure at all.
- **Fix (landed):** the container posts the working document to `/api/reports/render` and `buildBlockBody` maps the response per kind, returning `{ kind: "unavailable" }` — drawn as `—` — whenever the renderer supplied no value or reported a failure. Tests: `reports-exact-adapter.test.ts` "does not invent a figure when the renderer supplied none", "treats a failed widget as unavailable rather than empty".

### REPORTS-37 · HIGH · BLOCKED — Four palette kinds have no renderer contract
...
- **Design:** l.2554 draws a text block as three grey skeleton bars, and the block inspector (l.2242-2265) has no body field — the reference has no way to author the copy a Text block would contain.
- **Code:** ported literally, so a Text block dropped on the page renders the three bars and the operator cannot type into it. The stored `text` field survives round-trips but is never shown or edited on this surface.
- **Left open deliberately:** adding a body textarea is exactly the EXTRA that REPORTS-04 removes. Closing this needs a design decision about where text-block copy is authored, not a lint fix.

### REPORTS-39 · LOW · NOTE — The My reports footnote describes a send path that does not exist
...
- The `CTR · 28d` / `Frequency · 28d` pair reads a new `GET /api/meta/ads/series`
  over `meta_ad_daily`, which had stored the daily trail all along. Same
  `requireBusinessAccess({ minRole: "guest" })` gate as its sibling read routes,
  narrowed to the business's assigned Meta accounts. No new authority.

A note on how the BLOCKED rows are worded, after a verification pass corrected
three of them: a block must name the fact that is actually missing. Saying a
table "has no revenue" when it has a `revenue` column, or that `reach` exists
"nowhere" when it is stored, typed and served, sends the next implementer to
build a gap that is not there. DRAWERS-04, DRAWERS-20 and DRAWERS-39 below were
re-stated for that reason, and DRAWERS-20 turned out not to be blocked at all.
...
| DRAWERS-04 | BLOCKED | The `Placement mix` card exists with the design's geometry and three em-dashed rows. **Contract required:** a placement breakdown carrying a **campaign / ad set / ad key**. The gap is the entity key alone: `meta_breakdown_daily` is `UNIQUE (business_id, provider_account_id, date, breakdown_type, breakdown_key)` with no entity column (`lib/migrations.ts:7779-7809`), so every placement row is account-wide and a per-creative share is not a function of it. Money is **not** the gap — the DDL has `revenue` and `roas` columns and `MetaBreakdownDailyRow → MetaWarehouseBaseRow → MetaWarehouseMetricSet` carries both (`lib/meta/warehouse-types.ts:168-181, 649`); an earlier revision of this row claimed otherwise and was wrong. `lib/meta/entity-signals-backfill.ts:713` reports the same account-grain limit as `meta_breakdown_daily_has_no_campaign_or_adset_key`. |
...
| DRAWERS-20 | CLOSED  | *Re-filed, then built.* This was never absent data — it was a missing **read path**. `meta_ad_daily` already stored `date`, `ad_id`, `link_clicks`, `frequency` and `ctr` (`lib/migrations.ts:7818-7859`) behind `idx_meta_ad_daily_ad (ad_id, date DESC)` (`:7912`), and the batch already resolves creative → ads; nothing read it as a series. The read path is now `getMetaAdDailySeries` (`lib/meta/warehouse.ts`) → `GET /api/meta/ads/series` (same `requireBusinessAccess` gate as its siblings, narrowed to the business's assigned accounts) → `fetchCreativeEvidenceAdSeries` in `MetaPlatformPage`. Both cards draw a real 28-point path on the design's `0 0 100 22` geometry with a computed note (`link CTR −38.2% vs prior 14d`). The series is the decision's **own ad**, matching the Frequency evidence key beside it; a cross-ad frequency would need a deduplicated reach Meta does not report, so the route weights by impressions and never sums. |
...
| DRAWERS-39 | BLOCKED | *New finding.* `First-time reach` (design evidence key 2) stays em-dashed. **Contract required:** a decomposition of a day's reach into first-time and repeat people. Plain `reach` is **not** the gap and this row previously said it was, wrongly: `meta_ad_daily` has a `reach` column keyed on `ad_id` (`lib/migrations.ts:7840`), `CreativeWarehouseCommonFields` declares `reach?: number` (`lib/meta/creatives-types.ts:508`), and `lib/meta/creatives-service-support.ts:493, 654` populate it on every served row. What no served fact carries is the *first-time* share of that reach — neither Meta's insights payload nor this warehouse splits reach into new and repeat people, and `1 / frequency` is a different quantity (impressions per person, not people per exposure count). Publishing plain reach under this label would be exactly the substitution DRAWERS-40 refuses for Hold 15s. |
...
| DRAWERS-43 | ACCEPTED GEOMETRY DIVERGENCE | *New finding. Supersedes the batch's original "bar-scale note (not a defect)", which was wrong to file this as a non-issue: the funnel sub-bars were drawn 2–3× longer than the reference (68.0 / 48.3 / 40.7 against the design's 46 / 27 / 15).* The design's bars are a log scale and the scale **is** recoverable. Solving each of its nine authored widths for the decade span `D` in `1 + log10(v / top) / D` (design file 3610, 3624, 3638) gives 3.65 · 4.35 · 4.29 / 3.45 · 4.26 · 4.23 / 4.26 · 4.27 · 4.09 — seven of the nine cluster in 4.09–4.35, so the design draws a fixed window of decades below the funnel top. `funnelShare` now uses `D = 4.25`, landing those seven within 3 points of the authored width. **The accepted residual:** the two outliers are the `Clicks` bars of the Refresh and Retire funnels (`D` 3.65 and 3.45), which the design draws at 46% and 42% where its own scale gives 53.6% and 52.9%. They are hand-authored, not computed — no single monotone rule carries them with the other seven: fitting the Clicks bars alone needs a power-law exponent of 1.34 from Retire→Refresh and 0.55 from Refresh→Scale, or a log slope of 0.59 then 0.29 bar-fraction per e-fold. Fitting them would break the seven that agree, so the divergence is kept and recorded. **A second reason the old formula was wrong, independent of length:** `log10(v + 1) / log10(top + 1)` was not scale-free — it grew toward 100% as raw counts grew, so two funnels of identical shape drew differently purely by account volume. A decade window depends on `v / top` alone. The number printed beside each bar stays the literal served count either way. |
