    ...
    <div data-reports-surface="library" data-el="reports-lib">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
        <div>
          ...
          <p style={{ margin: "5px 0 0", fontSize: 12, color: "var(--ledger-ink-secondary)" }}>{copy.saveReportFormatsExportCsv}</p>
        </div>
        <Button
          variant="secondary"
          data-report-create=""
      ...
      <div data-report-library-columns="" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.05fr) minmax(0, 1.25fr)", gap: 12, alignItems: "start" }}>
        <section style={{ padding: 16, border: "1px solid var(--ledger-border-subtle)", borderRadius: 10, background: "var(--ledger-bg-surface)" }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 12, color: "var(--ledger-ink-tertiary)" }}>{copy.everySavedReportBelongsToBusiness}</p>
          {reports.length > 5 ? <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
            <input aria-label={copy.searchReports} value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={copy.searchReportsPlaceholder} style={{ minHeight: 34, minWidth: 220, padding: "6px 9px", border: "1px solid var(--ledger-border-control)", borderRadius: 6, background: "var(--ledger-bg-surface)" }} />
            <select aria-label={copy.sortReports} value={sortMode} onChange={(event) => setSortMode(event.target.value as "recent" | "name")} style={{ minHeight: 34, padding: "6px 9px", border: "1px solid var(--ledger-border-control)", borderRadius: 6, background: "var(--ledger-bg-surface)" }}><option value="recent">Sort: Recently updated</option><option value="name">Sort: Name</option></select>
          </div> : null}
          {reports.length === 0 ? <p data-reports="empty" style={{ marginTop: 16, padding: 20, border: "1px dashed var(--ledger-border-control)", borderRadius: 8, fontSize: 12, color: "var(--ledger-ink-tertiary)" }}>{copy.noReports}</p> : visibleReports.length === 0 ? <p style={{ marginTop: 16, fontSize: 12 }}>{copy.noReportsMatchSearch}</p> : <div data-collection="reports" role="list" aria-label={copy.reportsTitle} style={{ display: "grid", gap: 8, marginTop: 16 }}>
        ...
        <section style={{ padding: 16, border: "1px solid var(--ledger-border-subtle)", borderRadius: 10, background: "var(--ledger-bg-surface)" }}>
          <h2 style={{ margin: 0, fontSize: 15 }}>{copy.templateGallery}</h2><p style={{ margin: "4px 0 0", fontSize: 12, color: "var(--ledger-ink-tertiary)" }}>{copy.startWithOneClickStructure}</p>
      ...
      <ZeroBaseDialog open={deleteCandidate !== null} onOpenChange={(open) => { if (!open) setDeleteCandidate(null); }} title={copy.deleteSavedReport} description={deleteCandidate ? `This permanently deletes “${deleteCandidate.name}”. This action cannot be undone.` : null} confirmLabel="Delete report" confirmCtl="gated:REPORT-01 delete-confirm" destructive onConfirm={() => { if (!deleteCandidate) return; onDelete?.(deleteCandidate.id); setDeleteCandidate(null); }} />
