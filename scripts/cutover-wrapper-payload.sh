...
# repository path that does not exist there. `scripts/cutover-wrapper-package.sh`
# packages this file verbatim into `scripts/cutover-wrapper-payload.sh` (which
# the worker image carries, because the image copies `scripts/` while
# `.dockerignore` excludes `.github/`) together with `scripts/cutover-wrapper.manifest`
# recording its SHA-256. `.github/scripts/hetzner-remote.sh deliver_cutover_wrapper`
# extracts both out of the exact pinned image and installs them under
# `${APP_DIR}/cutover`. `assert_wrapper_integrity` below then re-hashes THIS FILE
# against that manifest before any phase runs, so a wrapper that was edited on
# the host, truncated by a failed copy, or carried over from another release
# refuses instead of driving a cutover nobody reviewed.
#
# TWO HOSTS
#
