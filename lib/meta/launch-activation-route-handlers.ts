 ...
 * one thing this handler must never do is call that live.
 *
 * The body lives in `lib/` rather than in the route for the same reason the
 * pause family's does: the Automation queue raises an activation row and has to
 * be able to forward an approval into THIS handler, with every gate in this
 * order, instead of growing a second activation path of its own.
 */
...
/**
 * What a caller holding a claim may add.
 *
 * `beforeProviderMutation` is fired once, after every gate and immediately
 * before the activation sequence's first provider call. Returning false
 * refuses: a caller that cannot record its own dispatch intent must not cause
 * an activation it could never account for afterwards.
 */
export interface MetaLaunchActivationHandlerOptions {
...
export async function handleMetaLaunchIntentActivateAction(
  request: NextRequest,
  context: { params: Promise<{ id: string }> },
  options: MetaLaunchActivationHandlerOptions = {},
) {
  /*
  ...
  */
  ...
  if (options.beforeProviderMutation) {
    const mayDispatch = await options.beforeProviderMutation();
    if (!mayDispatch) {
      return jsonError(
        409,
        "dispatch_marker_unavailable",
        "The caller could not record dispatch intent for this activation, so nothing was activated on Meta.",
      );
    }
  }
