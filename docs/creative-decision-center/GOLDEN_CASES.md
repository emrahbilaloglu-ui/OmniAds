...
| GC-042  | unlabeled campaign with would-be scale after kind-aware resolver                                                                               | Scale                   | diagnose_data       | review_only           | data_quality         | medium               | low                    | campaign_label_missing                   | mature           | diagnose_data                     |
...
| GC-047  | unlabeled creative with gate-emitted refresh signal                                                                                            | Refresh                 | diagnose_data       | review_only           | data_quality         | medium               | low                    | campaign_label_missing                   | mature           | diagnose_data                     |
...
- GC-042 and GC-047 were `Diagnose` until **ADR D097**. The guard overwrote a
  computed verdict with `diagnose`, which INVARIANTS.md forbids ("`diagnose` is
  written only where the canonical mapper has no typed verdict for the type at
  all — never as a replacement for one it does have"). The verdict is now
  preserved and the row stays `review_only` with `blockedActionType` set, so
  nothing became executable: only the erasure is gone. GC-036 is unchanged —
  it reaches the guard through the medium-trust branch, which already
  preserved the label.
